 ...
 * supported) (default is rgba16) -o, --output-path      output file path (output to stdout if not specified) -u, bytes
 * per array element, one of 8,16, (without, default is chosen per type, 32/16/8 for 32/16/(8 or 4))
 *
 * Flags:
 *   -h, --help
 *   -l, --palette          Rip the palette from a palettised PNG (should err if is not palettised) as rgba16; ignores
 * -f, print a warning -r, --raw              output only the raw bytes in specified -u
 *
 * Positional argument:
